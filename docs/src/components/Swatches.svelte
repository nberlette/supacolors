<script>
  ...
  const paletteEntries = palette =>
    Object.entries(colors[palette]).filter(([k, v]) => !paletteKeyBlacklist.includes(k));

  /**
   * Returns an array of color shades for the specified palette.
   * @param {string} palette - name of the target color palette (e.g. `amber`, `cyan`, etc.)
   */
  const paletteKeys = palette =>
    Object.keys(colors[palette]).filter(k => !paletteKeyBlacklist.includes(k));

  /**
   * Returns an array of color values for the specified palette.
   * @param {string} palette - name of the target color palette (e.g. `amber`, `cyan`, etc.)
   */
  const paletteValues = palette =>
    Object.entries(colors[palette])
    ...
    {#each paletteNames as palette, p (palette)}
      ...
      <div>
        <div class="flex flex-row justify-start space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <div
            class="bg-gradient-to-t from-[#d7d2eadd] to-[#fbf2ffdf] px-6 py-5 w-full rounded-lg ring-1 ring-inset ring-offset-violet-1200/20 border border-violet-1200/90 shadow-lg hover:shadow-xl transition-all duration-300 ease-in"
          >
            <div
              class="min-w-0 flex-auto grid grid-cols-6 xl:grid-cols-12 gap-x-4 gap-y-3 lg:gap-x-2 justify-items-stretch"
            >
              {#each paletteEntries(palette) as [shade, color], c (c)}
                <div class="space-y-1.5">
                  <clipboard-copy value={toHex(color)} title="{palette}-{shade} ({toHex(color)})">
                    <div
                      class="color-swatch h-12 min-w-[48px] w-full rounded ring-2 dark:ring-inset dark:ring-white/10 cursor-pointer hover:shadow-lg shadow-md opacity-100 hover:!bg-[{tc(
                        color
                      )
                        .saturate(20)
                        .darken(10)
                        .toHex8String()}] transition-all duration-300 ease-in relative group"
                      style:--tw-ring-color={adjustRingColor(color)}
                      style="background-color:{color};"
                    >
                      <svg
                        width="24"
                        height="24"
                        class="checkmark text-grass-A900 opacity-0 transition-opacity w-6 h-6 absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] duration-300 ease-in-out"
                        ><use href="/icons.svg#i-check" /></svg
                      >
                      <div
                        class="color-name h-12 aspect-square text-center opacity-100 transition-all duration-300 ease-in-out drop-shadow text-scale-300 text-2xs md:text-xs font-mono font-medium tracking-tight uppercase overflow-hidden relative"
                        style:color={adjustLabelColor(color)}
                      >
                        <svg
                          width="24"
                          height="24"
                          class="opacity-0 group-hover:opacity-100 transition-all duration-[400ms] ease-in cursor-pointer absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"
                          style:color={adjustLabelColor(color)}
                          ><use href="/icons.svg#i-clipboard-list" /></svg
                        >
                      </div>
                    </div>
                  </clipboard-copy>
              ...
              {/each}
              <div
                class="col-span-6 xl:col-span-12 mt-2 flex items-center font-mono text-sm justify-between text-slate-700 w-full"
              >
                ...
                <svg
                  ...
                  ><circle cx="1.5" cy="2" r="1.5" class="opacity-50" /><circle
                    cx="12.5"
                    cy="2"
                    r="1.5"
                    class="opacity-75"
                  /><circle cx="23.5" cy="2" r="1.5" class="opacity-100" /><circle
                    cx="34.5"
                    cy="2"
                    r="1.5"
                    class="opacity-75"
                  /><circle cx="45.5" cy="2" r="1.5" class="opacity-50" /></svg
                >
                <span class="flex-1 text-right"
                  >{palette}-{paletteShades[paletteShades.length - 1]}</span
                >
              </div>
            </div>
          </div>
